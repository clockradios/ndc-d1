...
async function execute_sql_transaction(state: State, statements: InStatement[]): Promise<ResultSet[]> {
    ...
    try {
        const response = await Promise.all(statements.map(async statement => {
            let payload: any = statement
            ...
            if (res.data.success) {
                return res.data.result.map((result: any) => {
                    ...
                    return {
                        columns,
                        columnTypes: columns.map(() => ''), // Assuming column types are not provided in the response
                        rows,
                        rowsAffected: result.meta.changes,
                        lastInsertRowid: result.meta.last_row_id ? BigInt(result.meta.last_row_id) : undefined,
                        toJSON() {
        ...
        if (axios.isAxiosError(error)) {
            ...
            throw new Forbidden(error.response?.data.errors[0].message);
        } else {
            console.error('Unexpected error:', error);
            throw new Forbidden('An unexpected error occurred');
        }
    }


    // } catch (e) {
    //     throw new Forbidden('Failed to execute SQL transaction', { error: `${e}` });
    // }
}
